...
public:
  Summit(GraphLib::SubHyperGraphMatching::DataHyperGraph *hyper_data_,
         ...
         SubHyperGraphMatchingOption filter_option);
...
Summit::Summit(GraphLib::SubHyperGraphMatching::DataHyperGraph *hyper_data_,
               GraphLib::SubHyperGraphMatching::PatternHyperGraph *hyper_query_,
               SubHyperGraphMatchingOption filter_option) {
  opt = filter_option;
  ...
  Initialize();
  InitTime.Stop();
  fprintf(log_to, "Time for Memory allocation: %.02lf\n", InitTime.GetTime());
  Timer AfterMemory;
  AfterMemory.Start();
  bool SuccessHCS = BuildHyperCandidateSpace();
  AfterMemory.Stop();

  fprintf(log_to, "Time for Builing HCS: %.02lf\n", AfterMemory.GetTime());
  fprintf(log_to, "HCSBuildingTime: %.02lf\n", AfterMemory.GetTime());
  ...
  fprintf(log_to, "#Traversed Nodes: %lld\n", traversed_node);
  fprintf(log_to, "#Embeddings: %lld\n", ans);
  fprintf(log_to, "#Removed by cleaning: %lld\n", conflict);
  fprintf(log_to, "#Removed by intersection: %lld\n", conflict2);
  fprintf(log_to, "#Cleaning checking: %lld\n", num_check_cleaning);
  fprintf(log_to, "#Intersection checking: %lld\n", num_check_intersection);
}
...
void Summit::PrintCSStatistics(int level, int stage) {
  ...
  for (int u = 0; u < hyper_query->GetNumHyperedges(); u++) {
    ...
    if (level >= 1) {
      ...
      fprintf(log_to, "[LOG] Vert %d(label=%d) : Matching %d vertices\n", u,
              label, cand_sz[stage][u]);
      if (level >= 2) {
  ...
  fprintf(log_to, "Number of vertices: %d \e[0m\n", num_vertices);
}
...
void Summit::Initialize() {
  ...
  int *cand_sz_1 =
      (int *)calloc(MaxStage * hyper_query->GetNumHyperedges(), sizeof(int));
  ...
  bool *in_queue_removed_1 = (bool *)calloc(hyper_query->GetNumHyperedges() *
                                                hyper_query->GetNumHyperedges(),
                                            sizeof(bool));
  for (int e = 0; e < hyper_query->GetNumHyperedges(); e++) {
...
inline bool Summit::BuildHyperCandidateSpace() {
  BuildInitialHCS();
  fprintf(log_to, "Complete to build the initial HCS.\n");
...
void Summit::BuildInitialHCS() {
  ...
  fprintf(log_to, "# hyperedges with the same label\n");
  for (int e = 0; e < hyper_query->GetNumHyperedges(); e++) {
    int e_label = hyper_query->GetHyperedgeLabel(e);
    ...
    for (int i = 0; i < cand_sz[0][e]; i++) {
      ...
      for (int en_idx = 0; en_idx < query_edge_adj[e].size(); en_idx++) {
        ...
        for (int v : hyper_data->GetHyperedge(f)) {
          if (hyper_data->GetVertexLabel(v) != query_edge_adj_label[e][en])
            continue;
...
void Summit::InitializeMemory() {
  ...
  cand_exist =
      (unsigned long long ***)calloc(MaxStage, sizeof(unsigned long long **));
  unsigned long long **cand_exist_1 = (unsigned long long **)calloc(
      MaxStage * hyper_query->GetNumHyperedges(), sizeof(unsigned long long *));
  ...
  unsigned long long *cand_nbr_exist_3 =
      (unsigned long long *)calloc(VqCuDuCun_64, sizeof(unsigned long long));
  ...
  fprintf(log_to, "Space for cand: %lld, cand_nbr: %lld\n",
          8 * MaxStage * VqCu_64, 8 * VqCuDuCun_64);
}
        ...
        for (int f_idx = 0; f_idx < cand_sz[0][e]; f_idx++) {
          ...
          for (int v : hyper_data->GetHyperedge(f)) {
            ...
            for (int fn : hyper_data->GetIncidentHyperedges(v)) {
              ...
              if (cell_signature->CheckMapping(en, fn)) {
                int e_idx = query_edge_adj_idx[en][e];
                cand_nbr_exist[e][f_idx][en_idx][fn_idx / 64] |=
                    (1ULL << (fn_idx % 64));
                cand_nbr_exist[en][fn_idx][e_idx][f_idx / 64] |=
                    (1ULL << (f_idx % 64));
  ...
  for (int e = 0; e < hyper_query->GetNumHyperedges(); e++) {
    for (int f = 0; f < cand_sz[0][e]; f++) {
      for (int en_idx = 0; en_idx < query_edge_adj[e].size(); en_idx++) {
        ...
        for (int fn_idx_64 = initial_cand_sz_64[en] - 1; fn_idx_64 >= 0;
             fn_idx_64--) {
          unsigned long long temp = cand_exist[0][en][fn_idx_64] &
          ...
          if (temp) {
            ...
            for (int i = 0; i < 64; i++) {
              if (temp & (1ULL << i)) {
                ...
                cand_nbr[e][f][en_idx][cand_nbr_sz[e][f][en_idx]++] = t;
              }
            }
          }
        }
  ...
  while (!queue_removed.empty()) {
    ...
    for (int i = cand_sz[stage][e] - 1; i >= 0; i--) {
      ...
      if (!RestoreLocalMatch(e, f, en_idx, stage)) {
        ...
        if (SwapAndPop(cand[e], cand_idx[e], cand_sz[stage][e], i) == 0) {
          EmptyQueue();
          return false;
        }
        refined = true;
  ...
  while (!something.empty()) {
    ...
    if (!(stage == 0 && sz == 0)) {
      ...
      for (int en_idx = 0; en_idx < query_edge_adj[divided_e].size();
        ...
        if (cand_sz[stage][en] < cand_nbr_sz[divided_e][f_idx][en_idx] / 2) {
          for (int i = cand_sz[stage][en] - 1; i >= 0; i--) {
            num_check_cleaning++;
            int fn = cand[en][i];
            int fn_idx = initial_cand_idx[en][fn];
            int e_idx = query_edge_adj_idx[en][divided_e];
            if (!(cand_nbr_exist[en][fn_idx][e_idx][f_idx / 64] &
                  (1ULL << (f_idx % 64)))) {
              SwapAndPop(cand[en], cand_idx[en], cand_sz[stage][en], i);
            }
          }
        } else {
          int sz1 = 0;
    ...
    if (valid) {
        ...
        for (int i = 0; i < unmapped_edges_sz; i++) {
          ...
          for (int j = cand_sz[stage][en] - 1; j >= 0; j--) {
            ...
            if (!poss) {
              ...
              int n = SwapAndPop(cand[en], cand_idx[en], cand_sz[stage][en],
                                 cand_idx[en][fn]);
              if (n == 0) {
                EmptyQueue();
                valid = false;
      ...
      for (int i = 0; i < unmapped_edges_sz; i++) {
        ...
        if (std::pair<int, long long>{-unmapped_deg[e], cand_sz[stage][e]} <
