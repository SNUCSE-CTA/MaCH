...
namespace GraphLib {
    namespace SubHyperGraphMatching {
        ...
        void DataHyperGraph::LoadDataGraph(std::string &index_file,
            ...
            while (!fin.eof()) {
                ...
                if (l == -1) {
                    ...
                    //                    fprintf(stderr, "Because we could not
                    //                    find the signature, we skip %lld
                    //                    bytes\n", skip_bytes);
                    continue;
                }
                //                fprintf(stderr, "Found %d matching
                //                hyperedges\n", num_hyperedges_by_signature);
            ...
            if (DEBUG) {
                for (int i = 0; i < vertex_compression_map.size(); i++) {
                    if (vertex_compression_map[i] >= 0) {
                        vertex_compression_map_inv[vertex_compression_map[i]] =
                            i;
                    }
                }
            }
        ...
        void DataHyperGraph::LoadDataGraph_old(std::string dataset,
                                               std::string path,
                                               PatternHyperGraph &P) {
            ...
            std::cerr << "Read " << fileSize(hyperedge_file.c_str())
                      << " bytes from " << hyperedge_file << endl;
            Timer readtimer;
            readtimer.Start();
            ...
            while (getline(fin, line)) {
                ...
                current_hyperedge.erase(std::unique(current_hyperedge.begin(),
            ...
            readtimer.Stop();
            std::cerr << std::fixed
                      << "Graph read Time: " << readtimer.GetTime() << '\n';
            Timer pretimer;
            pretimer.Start();
            num_vertex = 0;
            for (int i = 0; i < vertex_compression_map.size(); i++) {
            ...
            pretimer.Stop();
            std::cerr << std::fixed
                      << "Preprocessing Time: " << pretimer.GetTime() << '\n';
        }

        int DataHyperGraph::GetOrigVertex(int i) {
            if (vertex_compression_map_inv.empty()) {
                fprintf(stderr,
                        "GetOrigVertex called without DEBUG info built\n");
                exit(-1);
            }
