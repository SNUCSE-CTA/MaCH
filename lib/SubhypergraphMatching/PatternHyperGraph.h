...
public:
  ...
  int GetNECRepresentative(const int v) { return NEC_representative[v]; }
  ...
  inline int GetNECArity(const int e) { return ContainedNEC[e].size(); }
  ...
  inline int GetNECLabel(const int idx) { return (int)NEC_label[idx]; }
...
void PatternHyperGraph::BuildNEC() {
  ...
  for (int i = 1; i < GetNumVertices(); i++) {
    if (vertex_label[vertex_order[i]] != vertex_label[vertex_order[i - 1]] ||
  ...
  for (int i = 0; i < NEC_partition.size(); i++) {
    NEC_incidence_list[i] = incidence_list[NEC_partition[i].front()];
  ...
  for (int e = 0; e < GetNumHyperedges(); e++) {
    for (int u_idx = 0; u_idx < GetArity(e); u_idx++) {
      ContainedNEC[e].push_back(NEC_idx[GetContainedVertex(e, u_idx)]);
    }
    ...
    for (int n_idx = 0; n_idx < GetNECArity(e); n_idx++) {
      ...
      for (int e_idx_from_n = 0; e_idx_from_n < GetNECDegree(n);
        ...
        if (NEC_incidence_list[n][e_idx_from_n] == e) {
          NEC_inverse_hyperedge_index[e][n_idx] = e_idx_from_n;
          break;
        }
      }
    }
