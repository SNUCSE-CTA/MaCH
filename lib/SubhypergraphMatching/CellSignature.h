...
public:
  void Initialize() {
    ...
    dbit = (unsigned long long *)calloc(data->GetNumVertices(),
                                        sizeof(unsigned long long));
    uint64_t total = query->GetNumVertices() + data->GetNumVertices();

    fprintf(stderr,
            "Before label compression = %llu, After = %llu, AllocatedMem = "
            "%llu KB\n",
            query->GetNumVertexLabels() * (1ull << query->GetNumHyperedges()),
            total, total * 8 / 1024);

    data_profiles = (unsigned long long *)calloc(data->GetNumVertices(),
                                                 sizeof(unsigned long long));
    query_profiles = (unsigned long long *)calloc(query->GetNumVertices(),
                                                  sizeof(unsigned long long));
