...
namespace GraphLib {
    class HyperGraph {
       protected:
        ...
        // A[i][j] = k: Hyperedge i is k-th incidence hyperedge of it's j-th
        // vertex.
       ...
       public:
        ...
        inline std::vector<std::vector<int>> &GetHyperedges() {
            return hyperedges;
        }
    ...
    void HyperGraph::LoadHyperGraphDataset(std::string dataset,
                                           std::string path) {
    ...
    void HyperGraph::BuildNeighborIndex() {
        ...
        std::vector<int> num_seen_(
            std::max(GetNumVertices(), GetNumHyperedges()), 0);
        for (int i = 0; i < GetNumVertices(); i++) {
